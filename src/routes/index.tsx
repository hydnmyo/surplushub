...
function Index() {
  ...
  const valueProps = [
    {
      icon: Boxes,
      title: t("home.valueSurplusTitle"),
      body: t("home.valueSurplusBody"),
    },
    {
      icon: Search,
      title: t("home.valueNeedTitle"),
      body: t("home.valueNeedBody"),
    },
    {
      icon: Sparkles,
      title: t("home.valueConnectTitle"),
      body: t("home.valueConnectBody"),
    },
  ];
  const loopiPrompts = [
    t("home.loopiPromptPackaging"),
    t("home.loopiPromptPlastic"),
    t("home.loopiPromptWood"),
  ];
  const trustItems = [
    {
      icon: BadgeCheck,
    ...
    {
      icon: LineChart,
      title: t("home.trustTrackedTitle"),
      body: t("home.trustTrackedBody"),
    },
    {
      icon: Sparkles,
      title: t("home.trustReviewsTitle"),
      body: t("home.trustReviewsBody"),
    },
  ];
    ...
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:py-28">
          ...
          <p className="mt-5 max-w-2xl text-lg text-forest-foreground/80">{t("home.heroBody")}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Button size="xl" variant="hero" asChild>
              <Link to="/marketplace">
                {t("home.exploreMaterials")} <ArrowRight className="size-4" />
              </Link>
            </Button>
            <Button size="xl" variant="onDark" asChild>
              <Link to="/dashboard">{t("footer.sellSurplus")}</Link>
            </Button>
            <Button size="xl" variant="onDark" asChild>
              <Link to="/wanted">{t("home.postNeed")}</Link>
            </Button>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        <SectionHead
          ...
          action={{ to: "/marketplace" as const, label: t("home.browseMarketplace") }}
        />
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {CATEGORIES.map((c) => {
            const listingCount = listingCountByCategory(c.id);
            const listingKey =
              listingCount === 1 && language === "en"
                ? "home.listingCount"
                : "home.listingCountPlural";
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <p className="mt-3 text-xs font-medium text-primary">
                    {t(listingKey, { count: listingCount })}
                  </p>
                </div>
              </Link>
            );
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
        <SectionHead
          ...
          action={{ to: "/marketplace", label: t("home.viewAllMaterials") }}
        />
      ...
      <section className="mx-auto grid max-w-7xl gap-5 px-4 py-16 sm:px-6 lg:grid-cols-2">
        <div className="surface-card p-7">
          <Badge variant="soft">{t("home.wantedBadge")}</Badge>
          <h2 className="mt-4 font-display text-2xl font-semibold">{t("home.wantedTitle")}</h2>
          <p className="mt-2 text-sm text-muted-foreground">{t("home.wantedBody")}</p>
          ...
          <Button className="mt-6" asChild>
            <Link to="/wanted">{t("home.postRequirement")}</Link>
          </Button>
        </div>
        ...
        <div className="surface-card gradient-mint p-7">
          ...
          <h2 className="mt-4 font-display text-2xl font-semibold">{t("home.loopiTitle")}</h2>
          <p className="mt-2 text-sm text-muted-foreground">{t("home.loopiBody")}</p>
          ...
          <p className="mt-5 text-xs text-muted-foreground">{t("home.loopiHint")}</p>
        </div>
      </section>
